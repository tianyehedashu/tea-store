  ...
  try {
    ...
    for (const config of apiKeyConfigs) {
      ...
      if (existingChannels.length > 0) {
        ...
        logger.info(`   ✅ Using existing sales channel: ${config.salesChannelName}`);
      } else {
        // 创建新的销售渠道
        ...
        logger.info(`   ✅ Created new sales channel: ${config.salesChannelName}`);
      }
      ...
      logger.info(`   ✅ ${config.title} - API Key: ${apiKey.token.substring(0, 20)}...`);
    }
    ...
    results.forEach(result => {
      const envName = `NEXT_PUBLIC_MEDUSA_PUBLISHABLE_KEY_${result.type.toUpperCase().replace('-', '_')}`;
