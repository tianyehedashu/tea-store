...
import { clearAllCache, clearCacheByTag, clearProductCache, clearCacheByPath } from "@lib/util/cache-manager"
...
export async function POST(req: NextRequest) {
  try {
    // 验证权限
    const auth = req.headers.get("x-cache-token") || req.nextUrl.searchParams.get("secret")
    ...
    switch (body.action) {
      ...
      case "clear-tag":
        if (!body.tag) {
          return NextResponse.json({ ok: false, message: "missing tag parameter" }, { status: 400 })
        }
        await clearCacheByTag(body.tag)
        return NextResponse.json({ ok: true, message: `标签 ${body.tag} 缓存已清理` })
      ...
      case "clear-path":
        ...
        return NextResponse.json({ ok: true, message: `路径 ${body.path} 缓存已清理` })
